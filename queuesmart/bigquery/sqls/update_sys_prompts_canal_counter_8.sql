-- =============================================================================
-- UPSERT canal_counter_prompt v8_slim
...
Infierelo del contenido de ESTA transcripción (no del nombre del archivo):
- Tipo OP (orden / compromiso de pago): gestion_principal = PAGO_MATRICULA; o el motivo principal es pagar matrícula, pensión, voucher o medio de pago; o hay compromiso de pago / tipificación OP (SI con pago pendiente por concretar).
- Tipo AD (administrativa): gestion_principal = DOCUMENTOS_REGULAR, DOCUMENTOS_CONVALIDACION o RECORDATORIO_EXAMEN; trámite de examen, entrega/subida de documentos, convalidación, recordatorio o gestión de alumno en proceso sin argumentario de inscripción nueva.
- Tipo RA (retorno / retoma): tipo_contacto = SEGUIMIENTO con retoma explícita (''has ido evaluando'', ''la vez pasada'', ''ya te comenté'', continúa ficha/pago/documentos/charla ya iniciada) sin desarrollar sondeo de motivación como en primer contacto comercial.

La pregunta de motivación SÍ aplica en atención tipo AG (primer contacto comercial / orientación con argumentario de inscripción nueva): gestion_principal = INFORMACION_CARRERA con PRIMER_CONTACTO, salvo que caiga en otro ''No aplica'' abajo.
...
(5) Motivación: RA/AD/OP → sondeo_motivacion = "NA". Tipo AG: "SI" solo si hay pregunta de motivación en el audio (basta preguntar; no exijas respuesta ni acompañamiento). Sin pregunta → "NO".
...
SELECT
  ...
  STRPOS(prompt_text, 'REALIZA una pregunta orientada a conocer la motivación') > 0 AS motivacion_solo_pregunta,
  STRPOS(prompt_text, 'NO condiciona la marcación') > 0 AS motivacion_solo_accion_pregunta,
  STRPOS(prompt_text, 'Escenario B') > 0 AS rebate_escenario_b_no
