...
19. Atenciones RA/AD/OP y gestión sin inscripción nueva: sondeo_motivacion en "NA" (no "NO") aunque no aparezca la pregunta de motivación. Tipo AG: sondeo_motivacion = "SI" si hay pregunta de motivación en el audio (solo la acción de preguntar; no exijas respuesta ni acompañamiento). Sin pregunta → "NO". También pre_cierre, cierre_comercial y sentido_de_urgencia en "NA" cuando corresponda.
...
29. rebate: consulta informativa NO es objeción. Sin objeción real → objecion_1..3 = "NA" y rebate = "SI" (RA/DS). Si hubo objeción real y el asesor no la identificó/rebatió → objecion_* con texto y rebate = "NO" (nunca "NA" en RA/DS). PROHIBIDO descripción "no identifica objeciones" con marcación "NA" salvo venta SI.
...
34. rebate: recorre TODA la transcripción antes de objecion_1..3. Si hubo objeción real (''caro'', ''lo pienso'', ''papás'', ''nacional'', etc.) → rebate SI o NO, nunca NA. Omisión del asesor = "NO", no "NA".
...
36. cierre_comercial_descripcion: debe reflejar la evidencia concreta del audio; prohibido plantilla genérica de WhatsApp/seguimiento sin cita.

37. resumen_venta: aplica SOLO si tipificacion = SI. Si tipificacion = RA/DS → resumen_venta = NA. Si tipificacion = SI → resumen_venta = SI o NO (prohibido NA).
