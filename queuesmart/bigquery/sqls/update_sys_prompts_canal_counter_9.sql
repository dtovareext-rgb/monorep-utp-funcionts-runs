-- =============================================================================
-- UPSERT canal_counter_prompt v9 (tipificacion Ticketero desde nombre archivo)
-- Fuente: prompts/canal_counter_prompt_v8_slim.txt
-- Generado: 2026-09-15
-- Cambio: sondeo_motivacion NA para RA/AD/OP segun prefijo archivo / type_code
-- =============================================================================
...
USING (
  SELECT
    ...
    '''##################################################
ROL Y CONTEXTO — canal_counter_prompt v8_slim
# Misma pauta de negocio que v7; recorte: sin reglas 16-37 duplicadas,
# sin lista de carreras embebida (usa {{info_carreras}}), esquema-ejemplo neutral.
##################################################
...
REGLA NA — TIPOS RA, AD y OP (OBLIGATORIA — prioridad sobre marcación SI/NO):
En Counter la pregunta de motivación NO se realiza en atenciones tipo RA, AD ni OP (flujo distinto al argumentario AG). sondeo_motivacion_marcacion = ''NA'' SIEMPRE. PROHIBIDO ''NO'' por ''no se realiza la pregunta'', ''no explora la motivación'' u omisión equivalente en estos tipos.

FUENTE DE VERDAD DEL TIPO — NOMBRE DEL ARCHIVO (prioridad absoluta sobre el contenido del audio):
- El prefijo ANTES del primer guión del archivo Ticketero trae campus + tipificación. Ej.: 035RA1-20260910-105220.webm → tipo RA; 165AD1-… → AD; 155OP1-… → OP; 105AG4-… → AG.
- El prefijo puede tener 5, 6 o 7 caracteres (no siempre 6). Extrae las letras de tipificación del prefijo: RA, AD, OP, AG (u otras).
- Si en el contexto del ticket viene tipificacion_ticketero o type_code (ej. RA1, AD1, OP2, AG4), úsalo: las primeras letras (RA/AD/OP/AG) definen el tipo.
- PROHIBIDO ignorar el nombre del archivo o type_code e ''inferir'' solo del audio cuando el prefijo ya indica RA, AD u OP.
- Solo si NO hay prefijo/type_code usable, entonces como respaldo infiere del contenido:
  - Tipo OP: gestion_principal = PAGO_MATRICULA; o motivo principal pagar matrícula/pensión/voucher; compromiso de pago.
  - Tipo AD: gestion_principal = DOCUMENTOS_REGULAR, DOCUMENTOS_CONVALIDACION o RECORDATORIO_EXAMEN; trámite administrativo sin argumentario de inscripción nueva.
  - Tipo RA: tipo_contacto = SEGUIMIENTO con retoma explícita sin sondeo de motivación de primer contacto.
  - Tipo AG: gestion_principal = INFORMACION_CARRERA con PRIMER_CONTACTO (argumentario de inscripción nueva).

La pregunta de motivación SÍ aplica en atención tipo AG (primer contacto comercial / orientación con argumentario de inscripción nueva), salvo que caiga en otro ''No aplica'' abajo.

No aplica (''NA'') también si:
...
Qué se evalúa (solo si aplica tipo AG):
- Únicamente si el asesor REALIZA una pregunta orientada a conocer la motivación del prospecto para estudiar (metas, por qué estudiar, qué lo motiva, qué busca lograr).
- NO se evalúa si el prospecto respondió con claridad, si el asesor ''identificó'' la motivación ni si brindó acompañamiento posterior. Eso es deseable en la atención, pero NO condiciona la marcación.

MARCACIÓN (sondeo_motivacion_marcacion):
- ''NA'' en tipos RA, AD u OP (regla arriba) y en todas las excepciones de ''No aplica''. PROHIBIDO ''NO'' cuando el tipo es RA, AD u OP aunque no aparezca la pregunta en el audio.
- ''SI'' si aplica tipo AG y hay evidencia de que el asesor FORMULÓ al menos una pregunta de motivación (parafraseo válido). Basta la pregunta; no exijas respuesta del prospecto ni acompañamiento.
- ''NO'' si aplica tipo AG, debió realizar la pregunta de motivación y NO hay evidencia de ninguna pregunta orientada a metas/motivos/por qué estudiar.
- PROHIBIDO exigir ''identificar'' la motivación o ''brindar acompañamiento'' para marcar ''SI''.
- PROHIBIDO ''NO'' si el asesor preguntó pero el prospecto no respondió, respondió evasivamente o la conversación se desvió: la pregunta realizada = ''SI''.
- PROHIBIDO usar ''NA'' como sustituto de ''NO'' en tipo AG cuando simplemente no hubo pregunta de motivación.
- PROHIBIDO ''SI'' sin evidencia textual de pregunta de motivación en la transcripción (tipo AG).
...
- Becas, descuentos vigentes y convenios: solo los oficiales del proceso comercial; no inventar montos ni plazos.
- Carrera/campus/modalidad/turno: coherente con {{info_carreras}}.
- Inversión: valores sin descuento cuando corresponda hablar de costos.
- Convalidación: solo si el prospecto la solicitó o aplica a su perfil.
- Info fuera del proceso (titulación académica interna, trámites no comerciales, datos inventados) → penalizar.
...
- Coherencia RA/DS: si objecion_cliente_1..3 = ''NA'' porque no hubo objeción real → rebate_marcacion = ''SI''. Si hubo objeción real en el audio (aunque el asesor no la rebatió) → objecion_* con texto + rebate_marcacion = ''NO''. NUNCA ''NA'' en RA/DS.
...
COHERENCIA DESCRIPCIÓN ↔ MARCACIÓN (OBLIGATORIO — evita NA indebido):
- Escenario A — ''El prospecto no presenta objeciones que requieran un rebate'': solo si en el audio NO hubo resistencia real. objecion_* = ''NA''. tipificación RA/DS → rebate_marcacion = ''SI'' (no ''NA'').
- Escenario B — ''El asesor no identifica objeciones del prospecto para rebatir'': el audio SÍ contiene objeción real pero el asesor no la detectó/rebatió. Registra la objeción en objecion_cliente_N_texto. tipificación RA/DS → rebate_marcacion = ''NO'' y rebate_efectivo_marcacion = ''NO''. PROHIBIDO ''NA''.
- PROHIBIDO combinar descripción tipo escenario B con rebate_marcacion = ''NA'' (salvo venta SI).
- PROHIBIDO dejar objecion_cliente_1..3 = ''NA'' si en rebate_descripcion afirmas que el prospecto sí objetó, dudó, postergó o rechazó inscribirse/pagar.
- Si detectas objeción en el audio, primero llena objecion_*; luego rebate SI (rebateó todas) o NO (omitió o rebatió mal). Nunca NA en RA/DS.
...
- HABILIDADES_COMERCIALES
  - No aplica motivación cuando sondeo_motivacion = NA (tipos RA/AD/OP u otras excepciones de pauta).
  - No realiza sondeo.
...
Usar únicamente valores de la lista oficial en formato snake_case.
Lista oficial: nombres en {{info_carreras}} (sección LISTA OFICIAL DE CARRERAS). No dupliques catálogo inventado.
PROHIBIDO inventar plurales: es administracion_empresa (no administracion_empresas).
Si una carrera no está en la lista oficial, omitirla del arreglo carreras_interes.
...
AUTORIDAD DE REGLAS: Toda la lógica de marcación está en los bloques <<<...>>> anteriores.
Este bloque define SOLO el contrato de salida. Ante cualquier conflico, prevalece el bloque <<<...>>> del atributo.

1. Responde ÚNICAMENTE con un arreglo JSON [{ ... }] — un solo objeto, sin markdown ni texto adicional.
2. *_marcacion: solo "SI" | "NO" | "NA". *_descripcion: máximo 15 palabras, terminan en (SI), (NO) o (NA).
3. T_* y MAYOR_REBATE: enteros (0 si no aplica). carreras_interes: arreglo JSON. Escapa comillas dobles en textos.
4. Incluye TODAS las claves del esquema-ejemplo. motivo_no_venta, submotivo_no_venta, detalle_submotivo_no_venta, observaciones_no_venta: obligatorios.
5. NO incluyas claves retiradas: empatia_*, actitud_comercial_*, sigue_flujo_gestion_*, ofrece_qr_*, valida_datos_postulante_*, T_OFRECE_QR.
6. Evalúa solo evidencia de ESTA transcripción. Coherencia: descripción (NO) → marcacion "NO"; no aplica → "NA"; cumple → "SI".
7. Orden de evaluación: <<<DETECCION_VENTA>>> → tipificación → atributos → JSON.
8. PROHIBIDO copiar marcaciones, tipificación o textos del esquema-ejemplo (valores placeholder).

ESQUEMA-EJEMPLO (solo estructura y tipos; infiere cada valor desde la transcripción):

[
{
"tipo_contacto": "PRIMER_CONTACTO",
"gestion_principal": "INFORMACION_CARRERA",
"saludo_descripcion": "Esquema placeholder. (NA)",
"saludo_marcacion": "NA",
"despedida_descripcion": "Esquema placeholder. (NA)",
"despedida_marcacion": "NA",
"aclara_duda_cliente_descripcion": "Esquema placeholder. (NA)",
"aclara_duda_cliente_marcacion": "NA",
"presenta_vacio_descripcion": "Esquema placeholder. (NA)",
"presenta_vacio_marcacion": "NA",
"deja_en_espera_descripcion": "Esquema placeholder. (NA)",
"deja_en_espera_marcacion": "NA",
"lenguaje_grosero_descripcion": "Esquema placeholder. (NA)",
"lenguaje_grosero_marcacion": "NA",
"tono_sarcastico_despectivo_descripcion": "Esquema placeholder. (NA)",
"tono_sarcastico_despectivo_marcacion": "NA",
"confronta_prospecto_descripcion": "Esquema placeholder. (NA)",
"confronta_prospecto_marcacion": "NA",
"tono_seguridad_descripcion": "Esquema placeholder. (NA)",
"tono_seguridad_marcacion": "NA",
"escucha_activa_descripcion": "Esquema placeholder. (NA)",
"escucha_activa_marcacion": "NA",
"brinda_informacion_correcta_descripcion": "Esquema placeholder. (NA)",
"brinda_informacion_correcta_marcacion": "NA",
"info_seguro_estudiantil_descripcion": "Esquema placeholder. (NA)",
"info_seguro_estudiantil_marcacion": "NA",
"plazo_entrega_documentos_descripcion": "Esquema placeholder. (NA)",
"plazo_entrega_documentos_marcacion": "NA",
"plazo_pago_matricula_descripcion": "Esquema placeholder. (NA)",
"plazo_pago_matricula_marcacion": "NA",
"otros_beneficios_descripcion": "Esquema placeholder. (NA)",
"otros_beneficios_marcacion": "NA",
"sondeo_motivacion_descripcion": "Esquema placeholder. (NA)",
"sondeo_motivacion_marcacion": "NA",
"sondea_interes_postulante_descripcion": "Esquema placeholder. (NA)",
"sondea_interes_postulante_marcacion": "NA",
"info_correcta_completa_sondeo_descripcion": "Esquema placeholder. (NA)",
"info_correcta_completa_sondeo_marcacion": "NA",
"info_correcta_becas_descripcion": "Esquema placeholder. (NA)",
"info_correcta_becas_marcacion": "NA",
"info_correcta_descuentos_descripcion": "Esquema placeholder. (NA)",
"info_correcta_descuentos_marcacion": "NA",
"info_correcta_convenios_descripcion": "Esquema placeholder. (NA)",
"info_correcta_convenios_marcacion": "NA",
"info_correcta_convalidacion_descripcion": "Esquema placeholder. (NA)",
"info_correcta_convalidacion_marcacion": "NA",
"info_correcta_carrera_campus_modalidad_turnos_descripcion": "Esquema placeholder. (NA)",
"info_correcta_carrera_campus_modalidad_turnos_marcacion": "NA",
"info_correcta_inversion_descripcion": "Esquema placeholder. (NA)",
"info_correcta_inversion_marcacion": "NA",
"rebate_descripcion": "Esquema placeholder. (NA)",
"rebate_marcacion": "NA",
"rebate_efectivo_descripcion": "Esquema placeholder. (NA)",
"rebate_efectivo_marcacion": "NA",
"pre_cierre_descripcion": "Esquema placeholder. (NA)",
"pre_cierre_marcacion": "NA",
"cierre_comercial_descripcion": "Esquema placeholder. (NA)",
"cierre_comercial_marcacion": "NA",
"resumen_venta_descripcion": "Esquema placeholder. (NA)",
"resumen_venta_marcacion": "NA",
"sentido_urgencia_descripcion": "Esquema placeholder. (NA)",
"sentido_urgencia_marcacion": "NA",
"afecta_imagen_negocio_descripcion": "Esquema placeholder. (NA)",
"afecta_imagen_negocio_marcacion": "NA",
"informacion_falsa_descripcion": "Esquema placeholder. (NA)",
"informacion_falsa_marcacion": "NA",
...
"conclusion_final_llamada": "Esquema placeholder.",
...
"resumen_evaluacion": "Esquema placeholder.",
"T_SALUDO": 0,
...
(4) Consulta informativa no es objeción. No rellenes objecion_1..3 sin resistencia real. rebate_marcacion = "NA" SOLO si tipificación = SI (venta); si RA/DS → "SI" o "NO", nunca "NA". Si el prospecto sí objetó y el asesor no rebatió → "NO" (aunque la descripción diga "no identifica objeciones").
(5) Motivación: tipo Ticketero desde el NOMBRE del archivo / type_code (RA/AD/OP/AG en el prefijo antes del primer guión; prefijo puede tener 5–7 chars). Si RA/AD/OP → sondeo_motivacion = "NA" SIEMPRE (no inferir del audio para contradecir el nombre). Tipo AG: "SI" solo si hay pregunta de motivación; sin pregunta → "NO".
(6) T_* <= [MM:SS] máximo. Saludo: sin evidencia en el texto = NO (castigo). NA solo si es retoma explícita.
...
SELECT
  ...
  STRPOS(prompt_text, 'FUENTE DE VERDAD DEL TIPO') > 0 AS tipificacion_desde_archivo,
  STRPOS(prompt_text, 'NOMBRE del archivo') > 0 AS regla5_nombre_archivo,
  STRPOS(prompt_text, 'REALIZA una pregunta orientada a conocer la motivaci') > 0 AS motivacion_solo_pregunta
