...
REGLA DURA — omisión = '0', NUNCA 'NA':
- Si hubo atención humana post `Caso recibido por` con espacio para sondear (el asesor ya dio carrera, flyers/malla, precios, modalidades, horarios, examen, ficha u otra gestión comercial) y NO preguntó por qué quiere estudiar / metas / motivación:
  - score = '0'
  - descripcion = 'Asesor no consulta la motivacion (0)'
  - incluir `NO_PREGUNTA_MOTIVACION` en sondeo_clasificacion.
- CDE / inactividad al FINAL de un chat ya desarrollado NO convierte motivacion en NA. Si ya hubo argumentario o costos y nunca preguntó: '0'.
- PROHIBIDO score 'NA' con textos del tipo 'El asesor no explora la motivación del prospecto para estudiar'. Esa omisión es incumplimiento ('0'), no 'no aplica'.
...
Si motivacion.score = '0' (no preguntó motivación teniendo oportunidad): incluir SIEMPRE NO_PREGUNTA_MOTIVACION.
...
14. motivacion: si el asesor no sondeó la motivación teniendo conversación desarrollada (carrera/precios/horarios/examen), score="0" y descripcion 'Asesor no consulta la motivacion (0)'. NUNCA "NA" por CDE al final. NA solo si el cliente cortó antes de poder preguntar o si el asesor preguntó y no obtuvo respuesta.
...
19. vacios/abandono: espera de ficha, medio de pago, llamada ('te llamo', 'comunicarme con tu mamá') o respuesta del cliente = no es '0' del asesor. No midas tiempos con bot/flow pre-asignación.
20. actitud_comercial: NO penalices por invitar al corporativo ante inactividad.
21. informacion_complementaria: 'Seguro: S/ X' cumple seguro; fecha de pago de matrícula cumple ese plazo. Describe SOLO lo omitido (habitualmente documentos).
...
WHEN NOT MATCHED THEN
  ...
  VALUES (s.prompt_name, s.prompt_text, CURRENT_TIMESTAMP());

SELECT
  prompt_name,
  updated_at,
  LENGTH(prompt_text) AS chars,
  STRPOS(prompt_text, 'Asesor no consulta la motivacion') > 0 AS motivacion_cero,
  STRPOS(prompt_text, 'JSON completo') > 0 AS json_completo,
  STRPOS(prompt_text, 'te llamo') > 0 AS vacios_llamada,
