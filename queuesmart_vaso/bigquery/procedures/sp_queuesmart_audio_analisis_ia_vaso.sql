...
BEGIN
  ...
  SELECT
    base.* EXCEPT(prompt_base),
    CONCAT(
      base.prompt_base,
      '\n\n--- CONTEXTO TICKET ---\n',
      'archivo: ', IFNULL(base.source_file_name, base.file_name), '\n',
      'recordid: ', IFNULL(base.recordid, 'N/D'), '\n',
      'campus: ', IFNULL(base.campus_code, 'N/D'), '\n',
      'type_code: ', IFNULL(base.type_code, 'N/D'), '\n',
      'tipificacion_ticketero: ', IFNULL(base.tipificacion_ticketero, 'N/D'), '\n',
      'nota_tipificacion: El prefijo del archivo (antes del primer guion) define RA/AD/OP/AG. Prefijo puede tener 5-7 chars. Si tipificacion_ticketero es RA, AD u OP, sondeo_motivacion DEBE ser NA.'
    ) AS prompt
  FROM (
    SELECT
      h.process_date,
      ...
      h.campus_code,
      h.type_code,
      -- Tipificación Ticketero desde type_code o prefijo del nombre (5–7 chars antes del 1er '-').
      CASE
        WHEN REGEXP_CONTAINS(UPPER(IFNULL(h.type_code, '')), r'^RA') THEN 'RA'
        WHEN REGEXP_CONTAINS(UPPER(IFNULL(h.type_code, '')), r'^AD') THEN 'AD'
        WHEN REGEXP_CONTAINS(UPPER(IFNULL(h.type_code, '')), r'^OP') THEN 'OP'
        WHEN REGEXP_CONTAINS(UPPER(IFNULL(h.type_code, '')), r'^AG') THEN 'AG'
        WHEN REGEXP_CONTAINS(
          UPPER(REGEXP_EXTRACT(IFNULL(h.source_file_name, h.file_name), r'^([^-]+)')),
          r'RA'
        ) THEN 'RA'
        WHEN REGEXP_CONTAINS(
          UPPER(REGEXP_EXTRACT(IFNULL(h.source_file_name, h.file_name), r'^([^-]+)')),
...
(
  ...
  SELECT
    ...
    -- Post-LLM: tipificación Ticketero (type_code / prefijo archivo) manda sobre el modelo.
    CASE
      WHEN tipificacion_ticketero IN ('RA', 'AD', 'OP') THEN 'NA'
    ...
    CASE
      WHEN tipificacion_ticketero IN ('RA', 'AD', 'OP') THEN
        CONCAT('Tipificacion Ticketero ', tipificacion_ticketero, ': motivacion no aplica. (NA)')
      ELSE JSON_VALUE(evaluacion_json, '$[0].sondeo_motivacion_descripcion')
    END AS sondeo_motivacion_descripcion,
    CASE
